<script lang="ts">
  ...
  onMount(() => {
    ...
    (async () => {
      ...
      unlisten = await listen(
        'search-progress',
        (event: { payload: { files_found: number; dirs_scanned: number } }) => {
  ...
  async function runSearch() {
    ...
    try {
      const metadata = await invoke<Record<string, unknown>>('search_files', {
      ...
      const errMsg = typeof e === 'string' ? e : (e as Error)?.message || JSON.stringify(e);
...
<main class="container">
    ...
    <div class="input-row">
      <div class="input-wrapper">
        <input
          ...
          autocomplete="off"
          style="{inputStyle}{validationError
            ? 'border-color:#ef4444;outline:1px solid #ef4444;'
            : ''}"
        />
      ...
      <button class="btn" onclick={pickFolder} disabled={isSearching} style={btnStyle}
        >Browse</button
      >
    </div>
    {#if validationError}
    ...
    <div style="display: flex; gap: 10px; width: 100%;">
      ...
      {#if isSearching}
        <button
          class="btn"
          onclick={stopSearch}
          style="border-color:#ef4444;color:#ef4444;width:auto;"
        >
          STOP
        </button>
      {/if}
  ...
  {#if isCancelled || result || validationError}
    <div class="results-display">
      {#if isCancelled}
        <div
          style="padding: 12px; background: rgba(251, 191, 36, 0.15); border: 1px solid #fbbf24; border-radius: 6px;"
        >
      ...
      {:else if validationError}
        <div
          style="padding: 12px; background: rgba(239, 68, 68, 0.1); border: 1px solid #ef4444; border-radius: 6px;"
        >
      ...
      {:else if result}
        <div style="margin-bottom: 1rem;">
          ...
          {#if result.totalMatchingFiles === 0}
            <p
              style="color: #ef4444; font-weight: 600; padding: 10px; background: rgba(239, 68, 68, 0.1); border-radius: 4px; margin-top: 10px;"
            >
