...
- **Added v1.11.0**: The Lint & Check job now also enforces a Dependency Audit (`pnpm audit` + `cargo audit`) and verifies IPC Type Synchronization (`generate:types` + `git diff --exit-code`).
