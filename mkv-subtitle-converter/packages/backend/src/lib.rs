...
fn convert_srt_to_ass(input_path: &Path, output_path: &Path) -> io::Result<()> {
    ...
    writeln!(ass_file, "Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text")?;
                    ...
                    if parts.len() == 2 {
                        ...
                        while let Some(next_line) = lines.peek() {
                            ...
                            if let Some(actual_line) = lines.next() {
                                if !text.is_empty() { text.push_str("\\N"); }
...
fn purge_all_generated_assets(folder_path: &str) {
    ...
    if let Ok(entries) = fs::read_dir(dir) {
        for entry in entries.flatten() {
            let path = entry.path();
            if path.is_file() {
                let filename_str = path.file_name().map(|n| n.to_string_lossy().to_lowercase()).unwrap_or_default();
                if filename_str.ends_with(".srt") || filename_str.contains("_extracted.srt") || filename_str.ends_with(".ass") {
...
async fn process_mkv_directory(
    ...
    tokio::spawn(async move {
        ...
        'folder_loop: for folder_path in &paths_clone {
            ...
            let folder_files: Vec<_> = all_files.iter()
            ...
            for file_path in folder_files {
                ...
                let file_stem = file_path.file_stem().and_then(|s| s.to_str()).unwrap_or("video");
                let file_name_with_ext = file_path.file_name().and_then(|s| s.to_str()).unwrap_or("video.mkv");
                ...
                if let Ok(sidecar_probe) = app_handle.shell().sidecar("ffprobe") {
                    ...
                    if let Ok(probe_output) = output {
                        if let Ok(parsed_json) = serde_json::from_slice::<serde_json::Value>(&probe_output.stdout) {
                            if let Some(streams_array) = parsed_json["streams"].as_array() {
                                for stream in streams_array {
                                    if let Some(idx) = stream["index"].as_u64() {
                                        let codec = stream["codec_name"].as_str().unwrap_or("subrip").to_string();
                                        let mut lang = stream["tags"]["language"].as_str().unwrap_or("und").to_string();
                                        if lang.trim().is_empty() { lang = "und".to_string(); }
                                        ...
                                        subtitle_streams.push((idx as u32, codec, lang, title, default_flag, forced_flag));
                                    }
                                }
                            }
                        }
                ...
                let supported_codecs = vec!["subrip"];
                let valid_tracks: Vec<_> = subtitle_streams.iter()
                ...
                if valid_tracks.is_empty() {
                    if subtitle_streams.is_empty() {
                        let _ = on_progress.send(ProgressPayload {
                            event: "LogMessage".to_string(),
                            data: json!(format!("ℹ No internal subtitle tracks discovered inside {}", file_name_with_ext)),
                        });
                    }
                ...
                for (stream_index, detected_codec, language, title, default_flag, forced_flag) in valid_tracks {
                    if cancel_token.load(Ordering::SeqCst) {
                        tracking_loop_aborted = true;
                        break 'folder_loop;
                    }
                    ...
                    let sanitized_title = if title.trim().is_empty() {
                        ...
                        title.replace(|c: char| !c.is_alphanumeric() && c != '_', "_").replace(" ", "_")
                    };
                    ...
                    let mut created_tracks_paths = Vec::new();
                    created_tracks_paths.push(tmp_srt_path.clone());
                    created_tracks_paths.push(final_ass_path.clone());
                    ...
                    if let Ok(sidecar_ffmpeg) = app_handle.shell().sidecar("ffmpeg") {
                        if let Ok((mut rx, _child)) = sidecar_ffmpeg
                            .args(["-y", "-i", &input_mkv_arg, "-map", &map_track_arg, &output_srt_arg])
                            .spawn()
                        {
                    ...
                    if operation_success && !cancel_token.load(Ordering::SeqCst) {
                        if convert_srt_to_ass(&tmp_srt_path, &final_ass_path).is_ok() {
                            ...
                            local_conv_list.push(SubtitleMetadata {
                                ...
                                track_name: if title.is_empty() { format!("Track Index {}", stream_index) } else { title.clone() },
                                codec: detected_codec.clone(),
                            });

                            let _ = on_progress.send(ProgressPayload {
                                event: "LogMessage".to_string(),
                                data: json!(format!("✓ Extracted SRT: stream {} [lang: {}] -> {}", stream_index, language, ass_filename)),
                            });
                            ...
                            local_fail_list.push(format!("Conversion failed on stream {}: {}", stream_index, file_name_with_ext));
                        }
                    } else if !cancel_token.load(Ordering::SeqCst) {
                        local_fail_list.push(format!("FFmpeg sidecar extraction skipped stream {}: {}", stream_index, file_name_with_ext));
                    }
            ...
            if !local_conv_list.is_empty() {
                ...
                report.insert("timestamp", json!(Local::now().format("%Y-%m-%d %H:%M:%S").to_string()));
        ...
        if cancel_token.load(Ordering::SeqCst) || tracking_loop_aborted {
            let _ = on_progress.send(ProgressPayload { event: "Cancelled".to_string(), data: json!("Interrupted.") });

            tokio::time::sleep(std::time::Duration::from_millis(350)).await;

            for srt_path in &session_temporary_srts {
                if srt_path.exists() { let _ = fs::remove_file(srt_path); }
            }
            for path_str in &paths_clone {
            ...
            if let Some(first_path) = paths_clone.first() {
                if Path::new(first_path).join("converted_files.json").exists() { final_success_dir = first_path.clone(); }
                if Path::new(first_path).join("failed_files.json").exists() { final_failure_dir = first_path.clone(); }
            }
...
fn show_item_in_folder(path: String) -> Result<(), String> {
    ...
    if !path_buf.exists() {
        return Err(format!("The target report item path does not exist: {}", path));
    }
    ...
    #[cfg(target_os = "macos")]
    {
        std::process::Command::new("open").args(["-R", &path_buf.to_string_lossy()]).status().map_err(|e| e.to_string())?;
    }
    #[cfg(not(any(target_os = "windows", target_os = "macos")))]
    {
        if let Some(parent) = path_buf.parent() {
            std::process::Command::new("xdg-open").arg(parent.to_string_lossy().as_ref()).status().map_err(|e| e.to_string())?;
        }
    }
    Ok(())
}
...
#[tauri::command]
fn check_folder_reports(paths: Vec<String>) -> Result<std::collections::HashMap<String, FolderReportStatus>, String> {
    let mut results = std::collections::HashMap::new();
    for path_str in paths {
        let base_path = Path::new(&path_str);
        results.insert(path_str.clone(), FolderReportStatus {
...
pub fn run() {
    tauri::Builder::default()
        .manage(AppCancellationState { is_cancelled: Arc::new(AtomicBool::new(false)) })
        .manage(ActivePathsState { paths: Arc::new(Mutex::new(Vec::new())) })
