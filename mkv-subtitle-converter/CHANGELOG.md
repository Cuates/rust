...
- **Release Pipeline**: Implemented an automated GitHub Actions Release Pipeline to automatically compile and distribute cross-platform system installers (`.msi`, `.dmg`, `.AppImage`, etc.) via GitHub Releases.
...
- **Documentation**: Split the monolithic README.md into modular, domain-specific markdown files across `docs/`, `frontend/`, and `backend/` directories.
...
- **Backend Refactoring & Tests**: Extracted pure logic (like JSON and version string parsing) into testable functions (`parse_ffprobe_output` and `parse_sidecar_version`), decoupled from Tauri commands. Implemented state and integration tests using mocked Tauri setup to improve test coverage to ~21%. Fixed a macOS CI compilation error by updating `tauri::test::mock_context()` to conform to the new Tauri v2 API signature (passing `tauri::test::noop_assets()` instead of four arguments). Resolved CI integration test panics by replacing `blocking_lock()` on Tokio Mutexes with `.lock().await` inside async test runtimes, and omitting lifecycle `setup()` database initialisation checks since Tauri mock builders do not run setup hooks. Resolved `cargo fmt` formatting violations that were blocking the CI pipeline.
