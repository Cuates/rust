...
- **IPC Type Generation**: Implemented `specta` to automatically generate TypeScript Zod schemas (`frontend/src/lib/types/ipc.ts`) from Rust structs, enforcing frontend/backend type synchronization.
- **Strict Quality Gates**: Added `knip` for frontend dead-code analysis, `markdownlint` for docs, and a Dependency Audit (`pnpm audit` + `cargo audit`) to block CI on vulnerabilities.
- Refactored GitHub Actions CI/CD pipeline and introduced a Composite Action for DRY environment setup.
- Synchronized all project documentation and bumped the application version to `1.11.0`.
