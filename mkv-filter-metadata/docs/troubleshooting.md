...
| **SvelteKit SSR Build Failure ("parse is not exported by cookie")** | This is caused by an upstream release of the `cookie` package (e.g., `2.0.1`) dropping named exports that SvelteKit requires. Resolve this by modifying `pnpm-workspace.yaml` to enforce the global override `cookie: "0.6.0"`. Do not use `pnpm.overrides` in `package.json` or explicitly install it in the frontend, as this will cause `ERR_PNPM_OUTDATED_LOCKFILE` during CI `--frozen-lockfile` validation if a conflicting workspace policy exists. |
| **GitHub CI `xvfb-action` crashes with exit code 127** | This action wraps the `run` command immediately after `/usr/bin/xvfb-run`. Prefixing the inline command with a bash environment variable assignment (e.g., `VAR=false pnpm...`) will cause `xvfb-run` to crash because it misinterprets the assignment as an executable. Declare variables strictly inside the `env:` block of the GitHub Action step instead. |
