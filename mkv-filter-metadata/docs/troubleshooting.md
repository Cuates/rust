...
| **SvelteKit SSR Build Failure ("parse is not exported by cookie")** | This is caused by an upstream release of the `cookie` package (e.g., `2.0.1`) dropping named exports that SvelteKit requires. Resolve this by pinning the dependency in the root `package.json` (`"pnpm.overrides": { "cookie": "0.6.0" }`) and reinstalling it explicitly in `frontend/package.json`. |
