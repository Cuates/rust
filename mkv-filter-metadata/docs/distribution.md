...
> [!NOTE]
> **Headless UI Testing:** If you are testing the Tauri application locally on a headless Linux server or within a Docker container, you must start an Xvfb virtual frame buffer (e.g., `xvfb-run pnpm test`) to simulate a display for WebKit, otherwise the application will fail to initialize.
...
```bash
git tag mkv-filter-metadata-v2.5.0
```
...
```bash
git push origin mkv-filter-metadata-v2.5.0
```
...
```powershell
git tag -d mkv-filter-metadata-v2.5.0
git push origin --delete mkv-filter-metadata-v2.5.0
```
